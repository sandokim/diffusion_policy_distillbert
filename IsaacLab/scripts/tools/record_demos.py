...
def run_simulation_loop(
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while simulation_app.is_running():
            ...
            action = teleop_interface.advance().flatten()
            # Match action dimension to environment (pad/tilt for bimanual setups)
            target_dim = int(env.action_space.shape[0])
            if action.numel() != target_dim:
                if action.numel() < target_dim and target_dim % action.numel() == 0:
                    repeats = target_dim // action.numel()
                    action = action.repeat(repeats)
                elif action.numel() < target_dim:
                    padded = torch.zeros(target_dim, device=action.device, dtype=action.dtype)
